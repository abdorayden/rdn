...
int main(void)
{
    ...
    RDNState stack = {0};
    rdn_funcs_define("hey", "331 print", NULL, 1, 1);
    rdn_funcs_define_native("foo", f, NULL);
    rdn_evaluate_source(&stack, "1");
    hello(&stack);
    // evaluate_source(&stack, "hey call");
    // evaluate_source(&stack, "foo call print");
    rdn_evaluate_source(&stack, "\"hey im rayden\" foo call print");
