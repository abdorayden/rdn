...
int main(void)
{
    ...
    rdn_funcs_define_native("init-window", 
                rdn_init_window, NULL);


    rdn_funcs_define_native("close-window", 
                rdn_close_window, NULL);


    rdn_funcs_define_native("window-should-close", 
                rdn_window_should_close, NULL);

    rdn_funcs_define_native("begin-drawing", 
                rdn_begin_drawing, NULL);

    rdn_funcs_define_native("End-drawing", 
                rdn_end_drawing, NULL);


    rdn_funcs_define_native("clear-background", 
                rdn_clear_bg, NULL);

    rdn_funcs_define_native("draw-rect", 
                rdn_draw_rect, NULL);

    rdn_funcs_define_native("get-screen-width", 
                rdn_get_width, NULL);

    rdn_funcs_define_native("get-screen-height", 
                rdn_get_height, NULL);

    rdn_funcs_define_native("set-target-fps", 
                rdn_set_target_fps, NULL);

    char* src = rdn_read_file("main.rdn");
    rdn_evaluate_source(&stack, src);
    return 0;
}
